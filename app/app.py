...
def count_employment_time(row):
    ...
    date_of_termination = (
        datetime.strptime(row["DateofTermination"], "%m/%d/%Y")
        if isinstance(row["DateofTermination"], str)
        else datetime.strptime("04/01/2021", "%m/%d/%Y")
    )
...
def draw_graphs():
    ...
    df["DOB_Datetime"] = df["DOB_Datetime"].apply(
    ...
    ax.savefig(
        "static/images/perfscroreid.png", dpi=300, bbox_inches="tight", pad_inches=0
    )

    ax = sns.displot(df_zscored, x="PerfScoreID", y="Salary")
    ax.savefig(
        "static/images/perfscroreid2.png", dpi=300, bbox_inches="tight", pad_inches=0
    )

    sns.set_palette("tab10")
    ax = sns.lmplot(
        data=df_zscored, x="EngagementSurvey", y="Salary", line_kws={"color": "red"}
    )
    ax.savefig(
        "static/images/eng_surv3.png", dpi=300, bbox_inches="tight", pad_inches=0
    )

    ax = sns.relplot(data=df_zscored, x="AbsenceRate", y="Salary")
    ax.savefig(
        "static/images/abs_rate4.png", dpi=300, bbox_inches="tight", pad_inches=0
    )

    sns.set_theme(style="ticks")
    ax = sns.jointplot(data=df_absence_zscored, x="AbsenceRate", y="Salary", kind="hex")
    ax.savefig(
        "static/images/abs_rate5.png", dpi=300, bbox_inches="tight", pad_inches=0
    )

    ax = sns.displot(
        data=df_zscored, x="EmpSatisfaction", y="Salary", hue="MaritalDesc"
    )
    ax.savefig(
        "static/images/married_satisf6.png", dpi=300, bbox_inches="tight", pad_inches=0
    )

    fig, axes = plt.subplots(1, 2, sharey=True, figsize=(15, 7))
    axes = axes.flatten()
    sns.histplot(
        data=df_zscored[df_zscored["MaritalBinary"]],
        x="EmpSatisfaction",
        y="Salary",
        ax=axes[0],
    )
    sns.histplot(
        data=df_zscored[df_zscored["MaritalBinary"] == False],
        x="EmpSatisfaction",
        y="Salary",
        hue="MaritalDesc",
        ax=axes[1],
    ...
    fig.savefig(
        "static/images/marital_binary7.png", dpi=300, bbox_inches="tight", pad_inches=0
    )
